...
class Author(db.Model):
    ...
    def validate_phone_numnber(self, key, phone_number):
        if len(phone_number) !=10:
            raise ValueError("Phone number must be eaxctly 10 digits.")
...
class Post(db.Model):
    ...
    def validate_title(self, key, title):
        ...
        if not any(phrase in  title for phrase in clickbait_phrases):
            raise ValueError('Title must contain one of the following phrases: {", ".join(clickbait_phrases)}.')
        return title
